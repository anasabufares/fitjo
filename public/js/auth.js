...
function signinHTML() {
  ...
  if (signRole === "staff") {
    ...
    <div class="form-row"><label>${staffRole === "owner" ? tL("Owner password", "كلمة مرور المالك") : tL("Coach access key", "مفتاح دخول المدرب")}</label>
      <input id="staffCode" type="password" autocomplete="off" placeholder="${staffRole === "owner" ? "••••••••" : "CX-XXXX-XXXX"}"></div>
    ...
    <div class="note">${staffRole === "owner"
      ? tL("Owners sign in with the gym's admin password.", "يدخل المالك بكلمة مرور الإدارة.")
      : tL("Coaches sign in with an access key generated by the gym owner in the admin dashboard.", "يدخل المدرب بمفتاح دخول ينشئه المالك من لوحة الإدارة.")}</div>`;
  }
  return `
  <button class="auth-x" id="authX">✕</button>
