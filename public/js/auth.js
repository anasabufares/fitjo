...
function signinHTML() {
  ...
  if (signRole === "staff") {
    ...
    <div class="form-row"><label>${staffRole === "owner" ? tL("Owner access code", "رمز دخول المالك") : tL("Coach access key", "مفتاح دخول المدرب")}</label>
      <input id="staffCode" type="password" autocomplete="off" placeholder="${staffRole === "owner" ? "OW-XXXX-XXXX" : "CX-XXXX-XXXX"}"></div>
    ...
    <div class="note">${staffRole === "owner"
      ? tL("Gym owners sign in with the access code from the super-admin, and manage their members, coaches and employees.", "يدخل مالك النادي برمز الدخول من الإدارة العليا لإدارة الأعضاء والمدربين والموظفين.")
      : tL("Coaches sign in with an access key generated by the gym owner.", "يدخل المدرب بمفتاح دخول ينشئه مالك النادي.")}</div>`;
  }
  return `
  <button class="auth-x" id="authX">✕</button>
...
function staffEnter() {
  ...
  if (staffRole === "owner") {
    // Owner = their own dashboard, opened with an owner access code.
    sessionStorage.setItem("fj_owner_code", code);
